...
| AP116 | Usage | Filter Used / Referenced / Unused | Open the Usage column filter and pick each option | "Used (plays)" shows slots with audible usage; "Referenced, never trigged" shows slots with only silent references; "Unused" shows slots with none; Reset filters returns to All |
