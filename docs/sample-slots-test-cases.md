...
| AP125 | Pool Fix | Convert replaces the original | Convert an MP3 via the context menu | Conversion starts immediately without any modal; the file's Compat badge is replaced by a progress throbber until the conversion ends; a .wav with the same stem appears, the MP3 is gone, and the file lists refresh with up-to-date badges; the converted file's badge starts green and slowly fades to its normal color; converting a 48k WAV keeps the exact file name; failures are reported in an alert |
