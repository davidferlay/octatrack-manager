...
| AP140 | Global | Table modal parity | Open all four table modals: Missing Samples, its Review planned changes, Incompatible Audio Pool Samples, its Review planned changes | All four resize horizontally (left/right edges) and vertically (bottom edge) and all four have a "toggle columns" menu whose hidden columns are also dropped from the copied TSV; the table area is the only scroll region (no second scrollbar on the modal itself); finishing a resize drag over the dark overlay does not close the modal |
| AP141 | Pool Fix | Usage column on the Audio Pool pane | Open the Audio Pool page (or a project's Audio Pool pane) on a pool with a file referenced by one or more projects of the Set | The dest table shows a Usage column with the same blue ✓ N / gray ○ N badges, dash-for-unused styling, and sort-by-usage as the Sample Slots Usage column; a badge's popover lists each usage prefixed with its project name, e.g. "ProjectA · Bank A · Part 1 · T1 · Machine"; the column offers the same Used (plays) / Referenced, not triggered / Unused filter; like Compat, the column only appears once a pool root is known |
| AP142 | Pool Fix | Include all projects of set | Open the Audio Pool page's Tools tab on a set with at least one project containing an incompatible audio file not in the pool | An "Include all projects of set" checkbox (default checked) sits above "Review before applying changes"; when checked, the Status count/denominator, the incompatible-files list modal, and the review modal all include project-local incompatible files (Location column shows the project's own folder name, e.g. "MyProject/"); unchecking narrows all three back to pool-only without re-scanning; Execute fixes whichever set is currently shown, updating the owning project's own slot references and backing it up first, same as it already does for pool files |
| AP143 | Pool Fix | Fix Project Samples tool, context-menu convert, and per-tab health glyph | Open a project's Tools tab and select "Fix Project Samples"; separately, right-click an incompatible slot on the Flex/Static tab; separately, look at the Flex/Static toolbar with the Audio Pool pane closed and open | Tools tab: scans every sample slot referencing an incompatible audio file wherever it physically lives (this project's own directory or the shared Audio Pool) plus any other incompatible file found recursively in the project's own directory; with "Review before applying changes" checked (default), Execute shows a review screen before converting, otherwise it converts immediately; Execute always converts to 44.1 kHz WAV in place, backing up each modified project of the Set first, then updates every slot reference to the new file. Context menu: a slot row's right-click menu offers "Convert to Octatrack format", disabled unless Edit mode is on, the slot has a path, its file exists, and its compatibility isn't "compatible" (also disabled while that slot's own conversion is in flight); clicking it calls the same fix in place for just that one file and shows a "Converted to Octatrack format" (or a "Could not convert: ..." warning) toast without opening the Tools tab. Health glyph: the Flex/Static toolbar shows a per-tab count of incompatible slots as an orange wrench pill (click opens Tools > Fix Project Samples) or a green check when none are incompatible; the glyph is hidden while the Audio Pool pane is open on that tab |
