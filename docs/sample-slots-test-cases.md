...
| AP125 | Pool Fix | Convert replaces the original | Convert an MP3 via the context menu and Apply Changes on the review screen | A .wav with the same stem appears, the MP3 is gone, and the file lists refresh; converting a 48k WAV keeps the exact file name; the review screen lists File, Compat, Location and Action per file with search, sort and copy-to-clipboard |
