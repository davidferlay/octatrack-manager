...
| AP124 | Pool Fix | Context-menu convert on incompatible file | Right-click an incompatible pool file (pane or page) | "Convert to Octatrack format" is enabled; on a compatible file it is disabled with tooltip "Already Octatrack-compatible"; with N incompatible files selected the item reads "(N)" |
| AP125 | Pool Fix | Convert replaces the original | Convert an MP3 via the context menu | Conversion starts immediately without any modal; the file's Compat badge is replaced by a progress throbber until the conversion ends; a .wav with the same stem appears, the MP3 is gone, and the file lists refresh with up-to-date badges; converting a 48k WAV keeps the exact file name; failures are reported in an alert |
...
| AP135 | Pool Page | Pane health glyph from background scan | Open the Audio Pool page on a pool containing incompatible files, then on a clean pool | The pool is scanned in the background on page load; once done, the Audio Pool pane toolbar (next to the file count) shows an orange warning glyph (tooltip "N incompatible audio files found — click to fix") that opens the Tools tab when clicked, or a green check ("All audio pool files are compatible with Octatrack") that is not clickable; the glyph refreshes after a fix or after deleting pool files |
