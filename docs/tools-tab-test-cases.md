...
| PRG-01 | Purge Project Samples | Status button shows count and opens preview | Select "Purge Project Samples". Wait for scan | Status button shows "N unused audio files - of M scanned". Click it: "Unused Project Samples" modal opens listing Name/Origin/Size, read-only, no changes made |
...
| PRG-04 | Purge Project Samples | Delete mode forces review | "Delete files" is selected (default) | "Review before applying changes" checkbox is checked and disabled (cannot be turned off) |
| PRG-05 | Purge Project Samples | Move mode allows skipping review | Select "Move files to folder" | "Review before applying changes" checkbox becomes enabled, still checked by default; unchecking it and clicking Execute applies the move immediately with no review screen |
...
| PRG-08 | Purge Project Samples | Delete mode sends to Trash | Apply Changes with "Delete files" selected | Files/directories removed from their location and recoverable from the OS Trash/Recycle Bin; done summary shows "N file(s) and M director(ies) removed (X reclaimed)" |
...
| PRG-15 | Purge Audio Pool Samples | Appears in operation dropdown, status/preview | Open Audio Pool Tools tab, select "Purge Audio Pool Samples" | Status button shows "N unused audio files - of M scanned"; click opens "Unused Audio Pool Samples" preview modal |
