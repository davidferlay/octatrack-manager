...
| PRG-33 | Purge Project / Audio Pool Samples | Non-audio files are counted separately | Same setup as PRG-32, with 2 unused audio files and 3 non-audio files in the collapsed directory | Status button reads "2 unused audio files + 3 other files"; the directory row reads "(2 files + 3 other files)"; the audio headline count never absorbs the non-audio ones |
...
| PRG-35 | Purge Project / Audio Pool Samples | Review banner is centered and color-coded per action | Open the review modal once with "Move files to folder" selected, then again with "Delete files" | Move shows a centered orange banner reading "will be moved to <destination folder>"; Delete shows a centered red banner reading "will be sent to the Trash Bin". Both show the audio count, any non-audio count, and the item/directory/size line |
| PRG-36 | Purge Project / Audio Pool Samples | Confirmation counts files inside removed directories | Purge a plan whose only row is a directory collapsing 50 audio files | Done screen reads "50 audio files, across 1 directory, ..." - never "0 files and 1 directory removed" |
