...
| FMS-09 | Fix Missing Samples | Browse button — user directory search | Create missing files not in project/pool/siblings. Click Execute. Click "Browse..." in progress section | Directory picker opens. After selecting, new "User selection: DirName" step appears with found count. Browse button remains if files still missing |
| FMS-10 | Fix Missing Samples | Browse — tooltip shows full path | After browsing a directory, hover over "User selection: DirName" step | Tooltip shows full directory path |
