...
| PT38 | Trig View | Hide empty gates each grid | Enable "Hide empty" on a track with sample trigs but no rec trigs (Trigs set to "All") | The empty recorder grid is hidden while the track grid stays; with "Hide empty" off both grids show even when empty; if no grid has content the pattern card is hidden entirely |
...
| PT40 | Trig View | Chips gated by Trigs view | Set the Trigs control to "Track", then "Rec" | In Track view the Rec Trig / One-Shot Rec chips are disabled; in Rec view every non-recorder chip is disabled; "All" restores content-based gating |
...
| PT42 | Indicators | Swing amount in pattern header | Set a track's swing amount above 50 for one pattern only | That pattern's header shows a "Swing: N%" chip next to Length/Speed; patterns where the amount is 50 show no chip |
| **Keyboard Navigation** | | | | |
