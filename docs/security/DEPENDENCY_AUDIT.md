...
- **build**: potentially reachable only while building reviewed docs content;
- **serve**: Docusaurus development/preview server only, not the deploy build;
- **PDF**: `docs-to-pdf`/Puppeteer CI path against the locally built site;
- **none**: installed but the affected capability is not invoked by current
  build/PDF workflows.
...
| --- | --- | --- | --- |
| `minimatch@3.1.2` | `@docusaurus/core > serve-handler > minimatch` | `GHSA-3ppc-4f35-3m26` high, `>=3.1.3`; `GHSA-7r86-cg39-jmmj` high, `>=3.1.3`; `GHSA-23c5-xmqv-rm74` high, `>=3.1.4` | serve; crafted glob patterns are not accepted from site visitors. |
...
| `brace-expansion@1.1.12` | `@docusaurus/core > serve-handler > minimatch > brace-expansion` | `GHSA-f886-m6hf-6m8v` moderate, `>=1.1.13`; `GHSA-3jxr-9vmj-r5cp` high, `>=1.1.16`; `GHSA-mh99-v99m-4gvg` high, `>=1.1.17`; `GHSA-rgw5-rvv9-x895` high, `>=1.1.18` | serve; no untrusted glob pattern input. |
...
| `path-to-regexp@8.3.0` | `docs-to-pdf > express > router > path-to-regexp` | `GHSA-j3q9-mxjg-w52f` high, `>=8.4.0`; `GHSA-27v5-c462-wpq7` moderate, `>=8.4.0` | PDF; local Express route definitions are fixed and the service is not public. |
...
| `basic-ftp@5.2.0` | `docs-to-pdf > puppeteer > proxy-agent > get-uri > basic-ftp` | `GHSA-6v7q-wjvx-w8wg` high, `>=5.2.2`; `GHSA-chqc-8p9q-pq6q` high, no patched version reported; `GHSA-rp42-5vxx-qpwr` high, `>=5.3.0`; `GHSA-rpmf-866q-6p89` high, `>=5.3.1` | none/PDF; PDF targets localhost and no FTP URL or attacker proxy configuration is used. Isolate from untrusted URLs. |
| `follow-redirects@1.15.11` | `webpack-dev-server > http-proxy > follow-redirects` | `GHSA-r4q5-vmmm-2653` moderate, `>=1.16.0` | serve; no proxy target is configured for the deployed static build. |
...
| `ip-address@10.1.0` | `docs-to-pdf > puppeteer > proxy-agent > socks > ip-address` | `GHSA-v2v4-37r5-5v8g` moderate, `>=10.1.1`; `GHSA-mwp4-54f8-5fhr` high, `>=10.3.1` | none/PDF; no SOCKS proxy or attacker-controlled IP is used. |
| `webpack-dev-server@5.2.3` | `@docusaurus/core > webpack-dev-server` | `GHSA-79cf-xcqc-c78w` moderate, `>=5.2.4`; `GHSA-mx8g-39q3-5c79` moderate, `>=5.2.5`; `GHSA-f5vj-f2hx-8m93` moderate, `>=5.2.6`; `GHSA-m28w-2pqf-7qgj` moderate, `>=5.2.6` | serve; deployment uses `docusaurus build`, not a public dev server. |
| `ws@8.20.0` | `webpack-dev-server > ws` | `GHSA-58qx-3vcg-4xpx` moderate, `>=8.20.1`; `GHSA-96hv-2xvq-fx4p` high, `>=8.21.0` | serve; no public WebSocket server. |
| `uuid@8.3.2` | `webpack-dev-server > sockjs > uuid` | `GHSA-w5hq-g745-h8pq` moderate, `>=11.1.1` | serve; affected predictable values are not security identifiers in a shipped service. Major-version fix needs upstream review. |
| `qs@6.14.2` | `webpack-dev-server > express > body-parser > qs` | `GHSA-q8mj-m7cp-5q26` moderate, `>=6.15.2` | serve; no public request parser. |
...
| `shell-quote@1.8.3` | `webpack-dev-server > launch-editor > shell-quote` | `GHSA-w7jw-789q-3m8p` critical, `>=1.8.4`; `GHSA-395f-4hp3-45gv` high, `>=1.9.0` | serve; command injection requires the editor-launch endpoint. CI/deploy never starts the dev server. **Not reachable in current workflows.** |
| `joi@17.13.3` | `@docusaurus/types > joi` | `GHSA-q7cg-457f-vx79` moderate, `>=17.13.4` | build; validates reviewed configuration. |
| `launch-editor@2.13.2` | `webpack-dev-server > launch-editor` | `GHSA-v6wh-96g9-6wx3` moderate, `>=2.14.1` | serve; Windows UNC handling is absent from macOS CI/deploy and no public dev server runs. |
| `undici@7.24.6` | `docusaurus-search-local > cheerio > undici` | `GHSA-vmh5-mc38-953g` high, `>=7.28.0`; `GHSA-p88m-4jfj-68fv` moderate, `>=7.28.0`; `GHSA-vxpw-j846-p89q` high, `>=7.28.0`; `GHSA-hm92-r4w5-c3mj` high, `>=7.28.0`; `GHSA-g8m3-5g58-fq7m` low, `>=7.28.0`; `GHSA-pr7r-676h-xcf6` moderate, `>=7.28.0`; `GHSA-8xcm-r25x-g524` moderate, `>=7.29.0`; `GHSA-4cwx-7wf7-3272` high, `>=7.29.0`; `GHSA-m8rv-5g2x-5cg5` moderate, `>=7.29.0`; `GHSA-jr45-8vmc-qm54` moderate, `>=7.29.0`; `GHSA-v3r7-h72x-cjcm` moderate, `>=7.29.0`; `GHSA-35p6-xmwp-9g52` low, `>=7.28.0` | build; transitive Cheerio fetch capability is not called by the local-search indexing workflow. Recheck if remote content ingestion is added. |
| `http-proxy-middleware@2.0.9` | `webpack-dev-server > http-proxy-middleware` | `GHSA-64mm-vxmg-q3vj` moderate, `>=2.0.10` | serve; no proxy target in the static deployment. |
...
| `ws@7.5.10` | `webpack-bundle-analyzer > ws` | `GHSA-96hv-2xvq-fx4p` high, `>=7.5.11` | none/serve; analyzer server is not enabled in deployment. |
| `websocket-driver@0.7.4` | `webpack-dev-server > sockjs > faye-websocket > websocket-driver` | `GHSA-mp7j-qc5w-4988` moderate, `>=0.7.5`; `GHSA-xv26-6w52-cph6` critical, `>=0.7.5` | serve; denial of service requires an exposed SockJS/WebSocket server. CI/deploy never starts one. **Not reachable in current workflows.** |
...
| `body-parser@2.2.2` | `docs-to-pdf > express > body-parser` | `GHSA-v422-hmwv-36x6` low, `>=2.3.0` | PDF; localhost-only helper with fixed requests. |
| `body-parser@1.20.4` | `webpack-dev-server > express > body-parser` | `GHSA-v422-hmwv-36x6` low, `>=1.20.6` | serve; no public dev server. |
...
| `extract-zip@2.0.1` | `docs-to-pdf > puppeteer > @puppeteer/browsers > extract-zip` | `GHSA-jmr9-qjv8-65gv` high, no patched version reported | install/PDF; archive source is Puppeteer's pinned browser acquisition path, not user input. Re-evaluate Puppeteer/browser provisioning rather than overriding transitives. |
...
| `rand@0.7.3`, `rand@0.8.5` | `tauri > tauri-utils > rand` | `RUSTSEC-2026-0097` / `GHSA-cq8v-f236-94qc`, low; fixed `>=0.8.6` for the affected 0.7/0.8 line | Transitive Tauri utility use; no security decision in application code relies on the affected generator. **Potentially reachable, low severity.** | Monitor and remediate through Tauri. |
