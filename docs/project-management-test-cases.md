...
| PM-BK2 | Project list | Individual Projects groups ordered by size | Scan a location holding several directories that each contain multiple projects | Groups are listed biggest first (project count descending), not in path order. Groups with the same count fall back to the displayed directory name |
