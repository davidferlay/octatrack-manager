...
test.describe('Fix Project Samples - Usage and Slot columns', () => {
  ...
  test('the Fix Project Samples review modal shows Slot/File/Format/Bit/kHz/Usage/Action by default, Size and Location hidden', async ({ page }) => {
    ...
    await expect(reviewModal.locator('thead')).toContainText('File')
    await expect(reviewModal.locator('thead')).toContainText('Format')
    await expect(reviewModal.locator('thead')).toContainText('Bit')
    await expect(reviewModal.locator('thead')).toContainText('kHz')
